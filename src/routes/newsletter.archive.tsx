...
import {
  ...
  Activity, Sparkles, ChevronDown, X, ArrowLeft,
} from "lucide-react";
import { MobileAppHeader } from "@/components/MobileAppHeader";
import { SparkleFab } from "@/components/SparkleFab";
import { cn } from "@/lib/utils";
import { EDITIONS, COVER_BG, type Edition, type Category } from "@/lib/newsletter-data";
...
export const Route = createFileRoute("/newsletter/archive")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse every Resolven newsletter edition by year, topic and category." },
    ],
  }),
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-[1fr_320px]">
          <section className="animate-rise">
            ...
            <div className="mt-4 sm:mt-6 flex justify-center">
              <button className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-4 py-2 text-[12.5px] font-medium text-foreground/80 hover:border-primary/40">
          ...
          <aside className="hidden lg:block space-y-4">
            <SidebarCard title="Archive Stats">
            ...
            <SidebarCard title="Categories">
              <ul className="space-y-1.5">
                {POPULAR_TOPICS.map((t) => {
                  ...
                  return (
                    <li key={t.label}>
                      <button onClick={() => setTopic(active ? "all" : t.label)}
                        className={cn(
                          "flex w-full items-center gap-3 rounded-xl px-2 py-1.5 text-left hover:bg-muted/50 transition",
            ...
            <SidebarCard title="Subscribe" subtle="Get every edition in your inbox.">
...
function ViewToggle({ view, onChange }: { view: "grid" | "list"; onChange: (v: "grid" | "list") => void }) {
  return (
    <div className="inline-flex rounded-full border border-border bg-card p-0.5">
      {([["grid", Grid3x3], ["list", List]] as const).map(([key, Icon]) => {
        const active = view === key;
        return (
          <button key={key} aria-label={key === "grid" ? "Grid view" : "List view"} onClick={() => onChange(key)}
            className={cn(
    ...
    <article className="module-card overflow-hidden rounded-2xl p-0">
      <Link to="/newsletter/$id" params={{ id: edition.id }} className="block">
        <div className={cn("relative aspect-[4/3] bg-gradient-to-br p-4 text-white", COVER_BG[edition.cover])}>
          ...
          <h3 className="mt-3 font-display text-[1.05rem] sm:text-[1.15rem] font-semibold leading-tight tracking-tight line-clamp-3">
            {edition.title}
          </h3>
      ...
      <div className="p-3.5 sm:p-4">
        <div className="text-[12.5px] font-semibold text-foreground">{edition.title}</div>
        <div className="mt-0.5 text-[10.5px] text-muted-foreground">{edition.date} · {edition.readMin} min read</div>
        <p className="mt-1.5 text-[11.5px] leading-snug text-muted-foreground line-clamp-2">{edition.excerpt}</p>
        <div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground">
          ...
          <button onClick={onSave} className={cn("hover:text-foreground", saved && "text-primary")} aria-label="Save">
  ...
  return (
    <Link to="/newsletter/$id" params={{ id: edition.id }}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 text-[10.5px] text-muted-foreground">{edition.date} · {edition.readMin} min read</div>
        <p className="mt-1 text-[11.5px] text-muted-foreground line-clamp-1">{edition.excerpt}</p>
      </div>
...
function SidebarCard({ title, subtle, children }: { title: string; subtle?: string; children: React.ReactNode }) {
  return (
    <div className="surface rounded-2xl p-4 sm:p-5 animate-rise">
      ...
      {subtle && <p className="mt-1 text-[11px] font-light text-muted-foreground">{subtle}</p>}
...
function FilterSheet({
  ...
  const topics: (Category | "all")[] = ["all", "Sustainability", "Innovation", "People & Culture", "Operations", "Community", "Leadership"];
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-foreground/40 backdrop-blur-sm" onClick={onClose}>
      <div className="surface w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5 animate-rise" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {years.map((y) => <Chip key={y} active={year === y} onClick={() => onYear(y)}>{y}</Chip>)}
            </div>
          </div>
          <div>
            <div className="mb-2 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Topic</div>
            <div className="flex flex-wrap gap-2">
              {topics.map((t) => <Chip key={t} active={topic === t} onClick={() => onTopic(t)}>{t === "all" ? "All" : t}</Chip>)}
            </div>
          </div>
        </div>
